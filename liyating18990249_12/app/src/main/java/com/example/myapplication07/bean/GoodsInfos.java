...
public class GoodsInfos {
    ...
    public GoodsInfos(int pic_id, String title, String desc) {
        this.pic_id = pic_id;
        this.title = title;
        this.desc = desc;
        this.bPressed = false;
    ...
    public static ArrayList<GoodsInfos> getDefaultList() {
        ArrayList<GoodsInfos> listArray = new ArrayList<GoodsInfos>();
        for (int i = 0; i < listImageArray.length; i++) {
            listArray.add(new GoodsInfos(listImageArray[i], listTitleArray[i], listDescArray[i]));
        }
        return listArray;
    }
    ...
    private static String[] gridTitleArray = {"羽绒服", "衬衫", "裤子", "连衣裙",
            "短袖", "套装", "吊带衫", "上衣", "背心", "雪纺衫"};

    public static ArrayList<GoodsInfos> getDefaultGrid() {
        ArrayList<GoodsInfos> gridArray = new ArrayList<GoodsInfos>();
        for (int i = 0; i < gridImageArray.length; i++) {
            gridArray.add(new GoodsInfos(gridImageArray[i], gridTitleArray[i], null));
        }
        return gridArray;
    }
    ...
    public static ArrayList<GoodsInfos> getDefaultStag() {
        ArrayList<GoodsInfos> stagArray = new ArrayList<GoodsInfos>();
        for (int i = 0; i < stagImageArray.length; i++) {
            stagArray.add(new GoodsInfos(stagImageArray[i], stagTitleArray[i], null));
        }
        return stagArray;
    }
    ...
    public static ArrayList<GoodsInfos> getDefaultCombine() {
        ArrayList<GoodsInfos> combineArray = new ArrayList<GoodsInfos>();
        for (int i = 0; i < combineImageArray.length; i++) {
            combineArray.add(new GoodsInfos(combineImageArray[i], combineTitleArray[i], null));
        }
        return combineArray;
    }
    ...
    public static ArrayList<GoodsInfos> getDefaultAppi() {
        ArrayList<GoodsInfos> appiArray = new ArrayList<GoodsInfos>();
        for (int i = 0; i < appiImageArray.length; i++) {
            appiArray.add(new GoodsInfos(appiImageArray[i], appiTitleArray[i], null));
        }
        return appiArray;
    }
